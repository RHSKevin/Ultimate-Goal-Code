...
@TeleOp
public class Teleop_RedSide extends LinearOpMode {
    //webcam variables
    OpenCvCamera webcam;
 ...
 */
            ...
            switch(ledState) {
                 case START:
                     robot.ledStrip.setPattern(RevBlinkinLedDriver.BlinkinPattern.BREATH_RED);
                     telemetry.addData("LED Color", RevBlinkinLedDriver.BlinkinPattern.BREATH_RED);
            ...
            switch(shootState) {
                ...
                case SHOOT_ODOMETRY:
                    ...
                    distance = Math.sqrt(((74 - (poseEstimate.getX() - 2.75)) * (74 - (poseEstimate.getX() - 2.75))) + ((-36 - (poseEstimate.getY() - 5.5)) * (-36 - (poseEstimate.getY() - 5.5))));
                    targetSpeed = a * (distance * distance * distance) + b * (distance * distance) + c * (distance) + d;
                    robot.shooter.setVelocity(targetSpeed);
                    robot.shooter2.setVelocity(targetSpeed);
                    aimHeading = Math.toDegrees(Math.atan2((-36 - (poseEstimate.getY() - 5.5)), (74 - poseEstimate.getX() - 2.75))) + shootAngleOffset;
                    if (Target != null) {
            ...
            if(gamepad2.back)
            {
                drive.setPoseEstimate(new Pose2d(-63.5, -64, Math.toRadians(180)));
                isAligned = true;
            ...
            telemetry.addData("SHOT OFFSET: ", c);
            if(Target != null) {
                camDistance = (distanceA * (Target.center.x*Target.center.x)) + distanceB*Target.center.x + distanceC;
                ...
                telemetry.addData("New Cam Center", newCamCenter);
            ...
            telemetry.addData("List Size", targetList.size());
    ...
    static class TargetDetectionPipeline extends OpenCvPipeline {
        ...
        //Colors
        static final Scalar RED_UPPER_MIN = new Scalar(155, 140, 90);
        static final Scalar RED_UPPER_MAX = new Scalar(180, 255, 255);
        static final Scalar RED_LOWER_MIN = new Scalar(0, 150, 110);
        static final Scalar RED_LOWER_MAX = new Scalar(15, 255, 255);
        ...
        ArrayList<MatOfPoint> findContours(Mat input) {
            ...
            for (int i = 0; i < lookUpTable.cols(); i++) {
                lookUpTableData[i] = saturate(Math.pow(i / 255.0, 1.5) * 255.0);
            }
            lookUpTable.put(0, 0, lookUpTableData);
            Core.LUT(input, lookUpTable, input);
            Imgproc.cvtColor(input, hueMat, Imgproc.COLOR_RGB2HSV);
            Core.inRange(hueMat, RED_LOWER_MIN, RED_LOWER_MAX, thresholdMat1);
            Core.inRange(hueMat, RED_UPPER_MIN, RED_UPPER_MAX, thresholdMat2);
            Core.bitwise_or(thresholdMat1, thresholdMat2, thresholdMat2);
        ...
        void morphMask(Mat input, Mat output) {
            //expand borders through dilation
